export default {
  filters: {
    /**
     * 将占位符包含的id转换成name文本展示
     * @param {*} value
     * @param {*} selectKeywords
     * @param {*} placeholder
     * @param {*} context
     */
    parseValue (value, selectKeywords, placeholder, context) {
      let [startFlag, endFlag] = placeholder
      let text = value
      selectKeywords.forEach(match => {
        let reg = new RegExp(`\\${startFlag}${match.id}\\${endFlag}`, 'g')
        let name = context.getValue(match.id, selectKeywords, 'id', 'name')
        text = text.replace(reg, `${startFlag}${name}${endFlag}`)
      })
      return text
    }
  },
  computed: {
    flatKeywords () {
      let flatKeywords = []
      this.keywords.forEach(item => {
        if (item.childrens && item.childrens.length) {
          flatKeywords = flatKeywords.concat(item.childrens)
        }
      })
      return flatKeywords
    }
  },
  methods: {
    /**
     * 可用于在选中的所有keywords中选出符合目标值的选中项
     */

    getValue (val, selectKeywords, compareKey, getKey) {
      let obj = selectKeywords.find(item => item[compareKey] === val)
      return obj && obj[getKey]
    }
  }
}
